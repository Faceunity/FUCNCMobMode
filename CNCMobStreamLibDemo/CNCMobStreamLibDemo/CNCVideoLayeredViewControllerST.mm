...
unsigned long long mGetTickCountST()
{
    struct timeval tv;
...
@interface CNCVideoLayeredViewControllerST () <UIGestureRecognizerDelegate,UIPickerViewDelegate,UIPickerViewDataSource,CNCMobStreamAudioEngineDelegate, CNCMobStreamVideoEncoderDelegate, CNCCaptureVideoDataManagerDelegate, CNCMobStreamRtmpSenderDelegate,CNCDemoCaptureVideoDataSTDelegate,UITableViewDataSource,UITableViewDelegate> {
    CGFloat screen_w_;
    CGFloat screen_h_;
...
@property (nonatomic, retain) CNCDemoCaptureVideoDataST *capture_ST;
///图像显示类
...
@implementation CNCVideoLayeredViewControllerST
...
- (void)init_st_capture {
    ...
    if (self.has_video) {
        ...
        self.capture_ST = [[[CNCDemoCaptureVideoDataST alloc] init_with_preview:self.preview direct:self.capture_info.direction] autorelease];
        self.capture_ST.delegate = self;
        [self.capture_ST start_capture];
    }
...
- (void)setup_view {
    ...
    {
        UILabel *label = [[[UILabel alloc] initWithFrame:CGRectMake(10, 40, screen_w_*0.7, screen_h_*0.8)] autorelease];
        label.backgroundColor = [UIColor clearColor];
        label.textColor = [UIColor redColor];
        label.lineBreakMode = NSLineBreakByWordWrapping;
        label.numberOfLines = 0;
...
#ifdef CNC_DEMO_ST
    if (self.capture_ST) {
        [self.capture_ST stop_capture];
        self.capture_ST.delegate = nil;
    }
//    self.capture_ST = nil;
#endif
}
...
#pragma mark - CNCDemoCaptureVideoDataSTDelegate
...
- (void)video_capture_st:(CNCDemoCaptureVideoDataST*)capture buf:(void *)buf pix_width:(int)pix_width pix_height:(int)pix_height format:(CNCENM_Buf_Format)format time_stamp:(int64_t)ts {
    ...
    [self.videoEncoder inputFrameBuffer:buf pix_width:pix_width pix_height:pix_height format:format time_stamp:ts];
//    [self.rtc_kit send_frame_buf:buf pix_width:pix_width pix_height:pix_height time_stamp:0];
...
- (void)update_statictics_beauty_after {
    ...
    unsigned long long pts = mGetTickCountST();
