...
unsigned long long mGetTickCountLayeredViewController()
{
    struct timeval tv;
...
- (void)action_add_sei_json_str:(UIButton *)sender {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        [self.videoEncoder set_sw_encoder_push_time_stamp:json_str];
    });
}
...
- (void)video_capture_buf:(void *)buf pix_width:(int)pix_width pix_height:(int)pix_height format:(CNCENM_Buf_Format)format time_stamp:(long long)ts {
    ...
    pixelbufferAttributes = [NSDictionary dictionaryWithObjectsAndKeys:
                             ...
                             nil];
    /*
     使用CNCCaptureVideoDataManager类采集时
     硬编对应的采集格式是BGRA
     软编对应的采集格式是YUV
     */
    ...
    int encodeWidth = 0;
    int encodeHeight = 0;
    CNCENM_Buf_Format encodeFormat;
    ...
    if (format == CNCENM_buf_format_BGRA) {
        ...
        pixelBuffer = [self copyDataFromBuffer:buf toYUVPixelBufferWithWidth:imageWidth Height:imageHeight];
    ...
    if (self.is_pushing) {
        if (self.videoEncoder) {
            if ([self.record_manager is_doing_store] && store_type == CNCRecordVideoType_GIF){
                [self.record_manager store_gif_record:buf pix_width:pix_width pix_height:pix_height format:format time_stamp:ts];
            }
            [self.videoEncoder inputFrameBuffer:buf pix_width:pix_width pix_height:pix_height format:format time_stamp:(unsigned int)ts];
        }
    }
...
- (void)capture_sample_bufferRef_data:(CMSampleBufferRef)sample_buf time_stamp:(unsigned int)time_stamp{
    ...
    NSLog(@"capture_sample_bufferRef_data 没用的回调");
}

- (void)capture_pixel_bufferRef_data:(CVPixelBufferRef)pixelBuffer time_stamp:(unsigned int)time_stamp {
    NSLog(@"capture_pixel_bufferRef_data  没用的回调");
}

- (void)frame_RGBA:(CVPixelBufferRef)imageBuffer time_stamp:(long)time_stamp {
...
- (void)update_statictics_beauty_after {
    ...
    unsigned long long pts = mGetTickCountLayeredViewController();
