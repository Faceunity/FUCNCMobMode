...
- (void)renderItemsToBufferData:(void *)buf pix_width:(int)pix_width pix_height:(int)pix_height {
    static int frameID = 0 ;
    if (frameID > 100) {
        [[FURenderer shareRenderer] renderItems:buf inFormat:FU_FORMAT_BGRA_BUFFER outPtr:buf outFormat:FU_FORMAT_BGRA_BUFFER width:pix_width height:pix_height frameId:frameID items:items itemCount:sizeof(items)/sizeof(int) flipx:YES];
    }
    frameID ++ ;
}
